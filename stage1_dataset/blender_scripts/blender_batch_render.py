...
SHIP_MODELS = [
    r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\blender\blender\b_s_1.stl",
    r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\blender\blender\b_s_2.stl",
    r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\blender\blender\b_s_3.stl",
    r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\blender\blender\b_s_4.stl",
    r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\blender\blender\b_s_5.stl",
    r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\blender\blender\b_s_6.stl",
    r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\blender\blender\b_s_7.stl",
]

# Seabed texture blend files from PolyHaven
# Add more seabed blend files here as you download them
SEABED_TEXTURES = {
    "gravelly_sand": r"D:\phd\blendr\blender-20260224T050211Z-1-001\blender\gravelly_sand_4k.blend\textures\gravelly_sand_diff_4k.jpg",
    # "sand": r"C:\path\to\sand_diff_4k.jpg",
    # "rock":  r"C:\path\to\rock_diff_4k.jpg",
}

# Output directory
OUTPUT_DIR = r"D:\phd\blendr\blender_renders"
...
def setup_render_engine():
    ...
    # Enable GPU in preferences — Blender 5.0 compatible
    prefs = bpy.context.preferences.addons['cycles'].preferences
    # Try OPTIX (RTX), fall back to CUDA, then HIP (AMD)
...
def create_seabed(texture_png_path, seabed_name):
    ...
    # Load texture image — supports PNG and JPG
    try:
        img = bpy.data.images.load(texture_png_path, check_existing=True)
        img.colorspace_settings.name = "sRGB"
        node_tex.image = img
        print(f"  Texture loaded: {texture_png_path}")
    except Exception as e:
        print(f"  Texture load failed: {e}")
        node_bsdf.inputs["Base Color"].default_value = (0.55, 0.50, 0.42, 1.0)
...
def import_ship(stl_path):
    ...
    # STL import — compatible with Blender 4.x and 5.0
    try:
    ...
    # Input names compatible with Blender 4.x and 5.0
    for name in ["Metallic", "Metallic Factor"]:
        if name in bsdf.inputs:
            bsdf.inputs[name].default_value = 0.3
            break
    for name in ["Roughness", "Roughness Factor"]:
        if name in bsdf.inputs:
            bsdf.inputs[name].default_value = 0.6
            break
    ...
    # Enable shadow casting — attribute path changed in Blender 5.0
    try:
        ship.cycles.cast_shadow = True
    except AttributeError:
        ship.visible_shadow = True
    return ship
